...
namespace Game.Services.Security
{
    ...
    public partial class PermissionService : IPermissionService
    {
        ...
        /// </remarks>
        private const string PERMISSIONS_ALLOWED_KEY = "Game.permission.allowed-{0}-{1}";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string PERMISSIONS_PATTERN_KEY = "Game.permission.";
        ...
        protected virtual bool Authorize(string permissionRecordSystemName, CustomerRole customerRole)
        {
            ...
            {
                foreach (var permission1 in customerRole.CustomerRolePermissionRecordMapping)
            ...
            foreach (var permission in permissions)
            {
                ...
                foreach (var defaultPermission in defaultPermissions)
                {
                    ...
                    var defaultMappingProvided = (from p in defaultPermission.PermissionRecords
                        ...
                        select p).Any();
                    var mappingExists = (from p in customerRole.CustomerRolePermissionRecordMapping
                        where p.PermissionRecord.SystemName == permission1.SystemName
                        select p).Any();
                    if (defaultMappingProvided && !mappingExists)
                    {
                        permission1.CustomerRolePermissionRecordMapping.Add(new CustomerRolePermissionRecordMapping(customerRole.Id,permission1.Id));
                    }
                }
        ...
        public virtual bool Authorize(string permissionRecordSystemName, Customer customer)
        {
            ...
            var customerRoles = customer.CustomerCustomerRoleMapping.Where(cr => cr.CustomerRole.Active);
            foreach (var role in customerRoles)
